...
export class ProfileComponent implements OnInit {
  ...
  fetchData(): void {
    ...
    if (this.role === 'user') {
      ...
      this.adminService.getAdminData(this.userId!, this.token!).subscribe({
        next: (res: any) => {
          this.userOrEmployer = res.admin;
          this.userOrEmployer.password = '';
        },
        error: () => this.error = 'Failed to load admin data.',
        complete: () => this.loading = false
      });
    }
  }
    ...
    const update$ =
      this.role === 'user'
        ...
        : this.role === 'employer'
          ...
          : this.adminService.updateAdminData(this.userId!, updated, this.token!);
    ...
    const delete$ =
      this.role === 'user'
        ...
        : this.role === 'employer'
          ...
          : this.adminService.deleteAdmin(this.userId!, this.token!);
