...
export class DashboardComponent implements OnInit {
  role: 'user' | 'employer' | 'admin' | null = null;
  ...
  ngOnInit(): void {
    ...
    if (this.role === 'user') {
      ...
      console.log('Admin role detected, fetching admin data...: ', this.adminId);
      
      this.fetchAdminData();
      console.log('Admin role detected, but no specific admin logic implemented yet.');
    }
  }
  ...
  fetchAdminData() {
    ...
    this.adminService.getAdminData(this.adminId!, this.token!).subscribe({
      next: (res) => {
        this.admin = res.admin; console.log(this.admin);
      },
      error: (err) => console.error('Admin data error:', err),
      complete: () => (this.isAdminLoading = false),
    });
  }
}
